    ...
    <div id="transactions" className="space-y-4">
      ...
      <div className="flex flex-col sm:flex-row flex-wrap items-stretch sm:items-center gap-3">
        ...
        <div className="flex gap-2">
          <Input type="date" value={dateFrom} onChange={(e) => setDateFrom(e.target.value)} className="flex-1 sm:w-[140px]" aria-label="Date from" />
          <Input type="date" value={dateTo} onChange={(e) => setDateTo(e.target.value)} className="flex-1 sm:w-[140px]" aria-label="Date to" />
        </div>
            ...
            {filtered.length === 0 ? (
              ...
              filtered.map((txn) => (
                <TableRow key={txn.id}>
                  ...
                  {role === "admin" && (
                    <TableCell>
                      <button onClick={() => openEdit(txn)} className="rounded p-1 hover:bg-accent" aria-label={`Edit ${txn.description}`}>
      ...
      <TransactionForm open={formOpen} onClose={() => setFormOpen(false)} editTransaction={editingTxn} />
    </div>
  );
}
